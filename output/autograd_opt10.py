"""Adam continuation from exp 14, another 9 iters."""
...
def loss_fn(params):
    ...
    sim_data = web.run(sim, task_name="autodesign_opt10", verbose=False)
    amp = sim_data["mode"].amps.sel(mode_index=0, direction="+").values
    T = anp.abs(amp) ** 2
    return -anp.sum(T)


init_params = np.load("output/opt_params9.npy").copy()
...
np.save("output/opt_params10.npy", best_params)
