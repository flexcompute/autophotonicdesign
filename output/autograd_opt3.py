"""Continue: 16 interior widths, warm-start from 10-point exp 7 result.
Run 9 iterations of momentum SGD."""
...
def make_from_params(params):
    ...
    widths = WG_WIDTH_IN + anp.cumsum(deltas)  # length M+1
...
def loss_fn(params):
    ...
    sim_data = web.run(sim, task_name="autodesign_opt3", verbose=False)
    amp = sim_data["mode"].amps.sel(mode_index=0, direction="+").values
    T = anp.abs(amp) ** 2
    return -anp.sum(T)


# Warm-start from exp 7
x_exp7 = np.concatenate([np.linspace(TAPER_LENGTH / 11, xb * 10 / 11, 10), [xb]])
w_exp7 = np.array([1.45697, 1.66118, 1.89786, 2.13878, 2.40396,
                   2.61480, 2.83810, 3.04023, 3.22683, 3.39607, 3.53862])
x_interior_new = np.linspace(TAPER_LENGTH / (M + 1), xb * M / (M + 1), M)
widths_new = np.interp(np.concatenate([x_interior_new, [xb]]), x_exp7, w_exp7)
...
np.save("output/opt_params3.npy", best_params)
