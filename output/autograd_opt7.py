"""Continue from exp 11 state with more iterations (longer run)."""
...
def loss_fn(params):
    ...
    sim_data = web.run(sim, task_name="autodesign_opt7", verbose=False)
    amp = sim_data["mode"].amps.sel(mode_index=0, direction="+").values
    T = anp.abs(amp) ** 2
    return -anp.sum(T)


# warm start from exp 11 (opt_params6.npy)
init_params = np.load("output/opt_params6.npy").copy()
print(f"Total params: {len(init_params)}")

params = init_params.copy()
# Adam optimizer (more stable at small steps)
...
np.save("output/opt_params7.npy", best_params)
