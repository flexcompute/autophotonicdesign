"""Adam continuation: 9 more iterations from exp 12 state."""
...
def loss_fn(params):
    ...
    sim_data = web.run(sim, task_name="autodesign_opt8", verbose=False)
    amp = sim_data["mode"].amps.sel(mode_index=0, direction="+").values
    T = anp.abs(amp) ** 2
    return -anp.sum(T)


init_params = np.load("output/opt_params7.npy").copy()
...
np.save("output/opt_params8.npy", best_params)
