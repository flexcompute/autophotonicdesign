"""Adam continuation: another 9 iterations from exp 13."""
...
def loss_fn(params):
    ...
    sim_data = web.run(sim, task_name="autodesign_opt9", verbose=False)
    amp = sim_data["mode"].amps.sel(mode_index=0, direction="+").values
    T = anp.abs(amp) ** 2
    return -anp.sum(T)


init_params = np.load("output/opt_params8.npy").copy()
...
np.save("output/opt_params9.npy", best_params)
deltas = np.log(1 + np.exp(best_params))
Ws = WG_WIDTH_IN + np.cumsum(deltas)
Ws[-1] = min(Ws[-1], 5.0)
print(f"\nBEST T = {best_T:.5f}")
print("widths ({}):".format(len(Ws)))
