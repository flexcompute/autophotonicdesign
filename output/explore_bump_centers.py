...
def make_sim(mid_center, mid_sigma, late_center):
...
def main():
    mid_centers = [0.38, 0.45, 0.52]
    mid_sigmas = [0.14, 0.18, 0.22]
    late_centers = [0.72, 0.78, 0.84]
    sims = {}
    for mc in mid_centers:
        for ms in mid_sigmas:
            for lc in late_centers:
                name = f"bc_mc{int(mc*100):02d}_ms{int(ms*100):02d}_lc{int(lc*100):02d}"
                sim = make_sim(mc, ms, lc)
                if sim is None:
    ...
    batch = web.Batch(
        simulations=sims,
        folder_name="autophotonicdesign_explore14_bump_centers",
        num_workers=len(sims),
    )
    print(f"Running {len(sims)} bump-center candidates")
    data = batch.run(path_dir="output/explore14_bump_centers")
    ...
    Path("output/explore14_bump_centers_results.tsv").write_text(
        "rank\tname\tmetric\n"
        + "\n".join(
