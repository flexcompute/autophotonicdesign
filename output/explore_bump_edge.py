...
def main():
    ...
    for mid, late in variants:
        name = f"bedge_m{mid:+.2f}_l{late:+.2f}".replace("+", "p").replace("-", "n")
    ...
    batch = web.Batch(
        simulations=sims,
        folder_name="autophotonicdesign_explore13_bump_edge",
        num_workers=len(sims),
    )
    print(f"Running {len(sims)} edge bump candidates")
    data = batch.run(path_dir="output/explore13_bump_edge")
    ...
    Path("output/explore13_bump_edge_results.tsv").write_text(
        "rank\tname\tmetric\n"
        + "\n".join(
