...
def main():
    mids = [-0.46, -0.40, -0.34, -0.28, -0.22]
    lates = [0.02, 0.10, 0.18, 0.26, 0.34]
    sims = {}
    for mid in mids:
        for late in lates:
            name = f"sa_m{mid:+.2f}_l{late:+.2f}".replace("+", "p").replace("-", "n")
    ...
    batch = web.Batch(
        simulations=sims,
        folder_name="autophotonicdesign_explore16_shifted_amplitudes",
        num_workers=len(sims),
    )
    print(f"Running {len(sims)} shifted-amplitude candidates")
    data = batch.run(path_dir="output/explore16_shifted_amplitudes")
    rows = []
    ...
    Path("output/explore16_shifted_amplitudes_results.tsv").write_text(
        "rank\tname\tmetric\n"
        + "\n".join(
