...
def main():
    tips = [0.20, 0.21, 0.22, 0.24, 0.26]
    powers = [1.75, 1.90, 2.05]
    sims = {
    ...
    batch = web.Batch(
        simulations=sims,
        folder_name="autophotonicdesign_explore7_wide_tip",
        num_workers=len(sims),
    )
    print(f"Running {len(sims)} wider-tip profile candidates")
    data = batch.run(path_dir="output/explore7_wide_tip")
    ...
    Path("output/explore7_wide_tip_results.tsv").write_text(
        "rank\tname\tmetric\n"
        + "\n".join(
